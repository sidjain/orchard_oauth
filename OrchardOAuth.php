...
class OrchardOAuth {/*{{{*/
  ...
  public static $TO_API_ROOT = "http://api.theorchard.com/api";
  ...
  function accessTokenURL() { return self::$TO_API_ROOT.'/access_token'; }
  ...
  function __construct($consumer_key, $consumer_secret, $oauth_token = NULL, $oauth_token_secret = NULL) {/*{{{*/
   ...
   // syslog(LOG_WARNING, "[TwitterOAuth] Twitter OAuth object constructed: $this->token");
  }/*}}}*/
   ...
   */
  function oAuthRequest($url, $args = array(), $method = NULL, $flag= NULL) {/*{{{*/
    ...
    switch ($method) {
    	case 'GET': 
    		{
    			//syslog(LOG_WARNING, "[TwitterOAuth] oAuthRequest method GET");
    			if($flag!=NULL)
    	...
    	case 'POST': 
    		{
    			//syslog(LOG_WARNING, "[TwitterOAuth] oAuthRequest method POST");
  ...
  function http($url, $post_data = null) {/*{{{*/
  	//syslog(LOG_WARNING, "[TwitterOAuth] Entered http() with url:" . $url . " and data: " . print_r($post_data));
    $ch = curl_init();
...
//	curl_setopt($ch, CURLOPT_STDERR, $mydebug);
    ...
    // syslog(LOG_WARNING, "[TwitterOAuth] curl_getinfo with headers" . print_r(curl_getinfo($ch),true) . "\n");
  	if(curl_errno($ch))
	{
		//syslog(LOG_WARNING, "[TwitterOAuth] CURL ERROR: "  . curl_error($ch));
	}
    curl_close ($ch);
    //syslog(LOG_WARNING, "[TwitterOAuth] http returning status code: " . $this->http_status . "with response: " . $response	);
    return $response;
  }/*}}}*/
